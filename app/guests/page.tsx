...
export default function GuestsLandingPage() {
  ...
  const handleBecomeGuestSuccess = () => {
  ...
  const isAlreadyGuest = userData?.isGuest;
  const shouldShowSetupProfile = isLoggedIn && !isAlreadyGuest;
  return (
    <>
      {/* BecomeGuestModal */}
      {shouldShowSetupProfile && user && (
        <BecomeGuestModal
          isOpen={showBecomeGuestModal}
          onClose={() => setShowBecomeGuestModal(false)}
          onSuccess={handleBecomeGuestSuccess}
          userId={user.uid}
        />
      )}
                ...
                {!isLoggedIn && (
                  <p className="text-white/60">
                    ...
                    <Link href="/auth/login" className="text-purple-400 hover:text-purple-300 font-semibold underline">
                      Sign in to set up your guest profile
                    </Link>
                  </p>
                )}
              </div>
